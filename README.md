...
- **Depot:** eigene Käufe von Aktien und Fonds eintragen (Stückzahl, Kaufpreis, Datum, mehrere Käufe je Wert möglich), Übersicht zeigt je Position den aktuellen Wert und Gewinn/Verlust sowie den Gesamtwert, das eingesetzte Kapital und den Gesamtgewinn, **umschaltbar in Lira, Dollar oder Euro** (wie beim Kursverlauf), dazu ein **Verlaufschart "Wert im Zeitverlauf"** (Depot-Wert seit dem ersten Kauf, aus der gehaltenen Stückzahl je Tag und den historischen Kursen berechnet). Dazu **Bargeld** (Lira/Dollar/Euro) eintragen, das zum **Gesamtvermögen** dazugerechnet wird (im Verlaufschart nicht enthalten, da kein Kaufdatum bekannt ist). Je Position außerdem direkt auf der Aktien-/Fonds-Detailseite unter "Mein Bestand". Rein lokal auf dem Gerät gespeichert (wie die Watchlist), keine Anlageberatung.
...
`npm test` (440 Tests) prüft unter anderem:
